...
fn main() {
    ...
    let no_submodule_build = match std::env::var(ENV_NO_SUBMODULE_BUILD) {
        ...
        Err(_) => false,
    } || env!("CARGO_PKG_NAME") == "runtime-wrapper";

    if !no_submodule_build {
        println!("cargo::warning=building submodule 'runtime-wrapper'");
        let _ = Command::new("cargo")
            .args(&["build", "--release", "-p", "runtime-wrapper"])
            .env(ENV_NO_SUBMODULE_BUILD, "1") // Prevent recursive submodule build
            .stdout(Stdio::null())
            .output();
        println!("cargo::warning=built submodule 'runtime-wrapper'");
    }
}
